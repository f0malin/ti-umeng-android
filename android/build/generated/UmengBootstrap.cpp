...
#define TAG "com.biddecor.android.umeng"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Umeng_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Umeng.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::UmengBindings::lookupGeneratedInit(
...
static void Umeng_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Umeng_getBinding)->GetFunction());
}

static void Umeng_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::UmengBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry UmengBinding = {
	"com.biddecor.android.umeng",
	Umeng_init,
	Umeng_dispose
};
...
Java_com_biddecor_android_umeng_UmengBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.biddecor.android.umeng", &UmengBinding);
	titanium::KrollBindings::addExternalLookup(&(::UmengBindings::lookupGeneratedInit));
}
